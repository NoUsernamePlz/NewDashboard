...
import Bubblegraph from './Graphs/BubbleChart';
...
const Widget: React.FC<WidgetProps> = ({ icon, subheading, text }) => {
  return (
    <div className="flex flex-col items-center p-4 h-[20vh] w-[8vw] ">
      ...
      <div className="mt-3 text-center text-sm font-bold text-steelblue-200 h-[1.5vh]">{subheading}</div>
      <div className="font-extrabold md:mt-4 text-41xl xs:max-lg:text-[2rem] text-white h-[9vh] flex items-center justify-center xs:max-lg:mt-[5vh] ">{text}</div>
    </div>
  );
};
...
const Middleright: React.FC<MiddlerightProps> = ({ processedData, handleFilterClick }) => {
 ...
 const zoomOut = () => {
  ...
  return (
    <div>
    <div className="flex items-center justify-center w-[70vw] xs:max-lg:w-[90vw] relative">
      <div className="bg-gray-200 text-center p-4 w-[100%] h-[66vh] relative flex flex-col justify-center items-center xs:max-lg:mt-[3vh]">
        ...
        <div className="absolute top-0 h-full flex flex-col items-center justify-center">
          ...
          <Bubblegraph  data={processedData} />
          </div>
