...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className=" w-[100%] h-[6vh] flex items-center justify-between text-purple-blue bg-blueish-black shadow-md font-sans sticky top-0 left-0 z-100 py-1 ">
      {/* First div */}
      ...
      <ul className={`md:flex items-center  md:pb-0  absolute md:static list-none  opacity-100  left-0 right-0 w-[100vw] md:w-auto md:pl-0   xs:bg-blueish-black xs:max-md:w-[100%] transition-all duration-500 ease-in ${open ? 'top-[-10px] ':'top-[-490px]'} `}>
        {
          Links.map((link)=>(
            <li key={link.name} className='md:ml-8  text-xl md:my-0 my-5 nav'>
              <a href={link.link} className=' hover:text-white duration-500 no-underline  text-purple-blue'>{link.name}</a>
            </li>
          ))
        }</ul>
        </div>
